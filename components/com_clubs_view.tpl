...
{if $can_create}
	<div class="new_club">
		{$LANG.YOU_CAN} <a href="javascript:void(0)" onclick="clubs.create(this);return false;"><i class="icon-plus-sign"></i> {$LANG.TO_CREATE_NEW_CLUB}</a>
	</div>
{/if}
		...
		<div class="club_entry{if $club.is_vip}_vip{/if}">
			...
			<div class="data">
				...
				<div class="details">
                    ...
                    {/if}
   					<span class="rating"><strong><i class="icon-star"></i> {$LANG.RATING}</strong> &mdash; {$club.rating}</span>
					<span class="members"><strong><i class="bac-users"></i> {$club.members_count|spellcount:$LANG.CLUB_USER:$LANG.CLUB_USER2:$LANG.CLUB_USER10}</strong></span>
                    <span class="date"><strong><i class="bac-date"></i> {$club.fpubdate}</strong></span>
				</div>
			</div>
		</div>
	{/foreach}
