...
{if $user_can_add}
	...
	<form action="/comments/{$do}" id="msgform" method="POST">
        ...
        {if !$is_user}
            <div class="cm_guest_name"><label>{$LANG.YOUR_NAME}: <input type="text" maxchars="20" size="30" name="guestname" value="" id="guestname" /></label></div>
        ...
        {if $is_user && $do=='add'}
            {if !$user_subscribed}
                <div class="cm_label">
                    <label style="padding:5px"><input name="subscribe" type="checkbox" value="1" style=" vertical-align:top" /> {$LANG.NOTIFY_NEW_COMM}</label>
                </div>
            {/if}
        {/if}
