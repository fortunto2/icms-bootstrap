    ...
    <tr>
        ...
        <td valign="top">
            <div class="data">
                <div class="details">
                    {if $club.is_vip}
                        <span class="vip"><strong><i class="bac-vipzone"></i> {$LANG.VIP_CLUB}</strong></span>
                    {else}
                        <span class="rating"><strong><i class="icon-star"></i> {$LANG.RATING}:</strong> {$club.rating}</span>
                    {/if}
                    <span class="members"><strong><i class="bac-users"></i> {$club.members+1|spellcount:$LANG.CLUB_USER:$LANG.CLUB_USER2:$LANG.CLUB_USER10}</strong></span>
                    <span class="date"><i class="bac-date"></i>{$club.fpubdate}</span>
                </div>
                ...
                <div class="clubmenu">
                    {if $is_admin}
                        <div class="config"><i class="icon-wrench"></i> <a  href="/clubs/{$club.id}/config.html">{$LANG.CONFIG_CLUB}</a></div>
                    	<div class="messages"><i class="icon-envelope"></i> <a class="ajaxlink" href="javascript:void(0)" onclick="clubs.sendMessages({$club.id});return false;" title="{$LANG.SEND_MESSAGE_TO_MEMBERS}">{$LANG.SEND_MESSAGE}</a></div>
                    {/if}
                    {if $user_id}
                        {if ($is_member || $is_admin || $is_moder) && $club.clubtype=='public'}
                        	<div class="invite"><i class="icon-random"></i> <a class="ajaxlink" href="javascript:void(0)" onclick="clubs.intive({$club.id});return false;">{$LANG.INVITE}</a></div>
                        {/if}
                        {if $is_member}
                        	<div class="leave"><a class="ajaxlink" href="javascript:void(0)" onclick="clubs.leaveClub({$club.id}, '{csrf_token}');return false;"><i class="bac-dismiss"> </i> {$LANG.LEAVE_CLUB}</a></div>
                        {elseif $club.admin_id != $user_id}
                        	<div class="join"><a class="ajaxlink" href="javascript:void(0)" onclick="clubs.joinClub({$club.id});return false;"><i class="bac-invite"> </i> {$LANG.JOIN_CLUB}</a></div>
                        {/if}
                    {/if}
                </div>
                {/if}
            ...
            <div class="clubcontent">
                ...
                <div class="album">
                    <div class="title"><i class="icon-picture"></i> {$LANG.PHOTOALBUMS}</div>
                    <div class="content">
            ...
            <div class="wall">
                <div class="header">
                    ...
                    <div class="club_wall_addlink">
                        <a href="javascript:void(0)" id="addlink" class="ajaxlink" onclick="addWall('clubs', '{$club.id}');return false;"><i class="icon-plus-sign"></i> 
                            {$LANG.WRITE_ON_WALL}
                        </a>
                    </div>
                </div>
