        ...
        <table cellpadding="0" cellspacing="0" border="0">
			...
			<td valign="top" style="padding-left:6px">
                        <div class="con_rss_icon">
                            <a href="/rss/content/{$cat.id}/feed.rss" title="{$LANG.RSS}"><i class="bac-share"> </i></a>
                        </div>
                </td>
        </table>
    {else}
...
{if $subcats}
	<div class="categorylist">
		{foreach key=tid item=subcat from=$subcats}
            <div class="subcat">
                <i class="icon-folder-open"></i>  <a href="{$subcat.url}" class="con_subcat">{$subcat.title}</a> ({$subcat.content_count})
