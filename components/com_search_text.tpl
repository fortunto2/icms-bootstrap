...
<div class="form-inline">   
    ...
    <input type="submit" class="btn" value="{$LANG.FIND}"/>
...
{if $results}
	...
	<p class="usr_photos_notice"><strong><i class="icon-info-sign"></i> {$LANG.FOUND} {$total|spellcount:$LANG.1_MATERIALS:$LANG.2_MATERIALS:$LANG.10_MATERIALS}</strong></p>
...
{else}
	{if $query}
		<p class="usr_photos_notice"><i class="icon-info-sign"></i> {$LANG.BY_QUERY} <strong>"{$query}"</strong> {$LANG.NOTHING_FOUND}. <a href="{$external_link}" target="_blank">{$LANG.FIND_EXTERNAL}</a></p>
    {/if}
{/if}
{literal}
