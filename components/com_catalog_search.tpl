...
<div class="uc_search_in_cat"><i class="icon-folder-open"></i> <a href="/catalog/{$cat.id}">{$cat.title}</a>
</div>
...
<form action="/catalog/{$id}/search.html" name="searchform" method="post" >
	...
	<p>
		<input type="submit" class="btn" name="gosearch" value="{$LANG.SEARCH_IN_CAT}" />
		<input type="button" class="btn" onclick="window.history.go(-1);" name="cancel" value="{$LANG.CANCEL}" />
	</p>
</form>