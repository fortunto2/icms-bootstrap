...
{if $do == 'newthread'}
    ...
    <div class="forum_postinfo"><table width="100%" cellpadding="5">
        <tr>
            ...
            <td width=""><input type="text" name="title" id="title" value="{$thread.title|escape:html}" /></td>
        </tr>
        <tr>
            <td>{$LANG.THREAD_DESCRIPTION}:</td>
            <td width=""><input type="text" name="description" value="{$thread.description|escape:html}" /></td>
        </tr>
    </table></div>
{/if}
...
<div class="forum_fa">
  ...
  <div class="forum_fa_entries" id="pa_entries" {if $thread_poll}style="display: block"{/if}>
        ...
        <div style="margin-bottom:10px"><table cellspacing="0" class="forum_fa_entry" cellpadding="5">
            <tr>
                <td>{$LANG.QUESTION}: </td>
                <td><input name="poll[title]" type="text" size="30" value="{$thread_poll.title|escape:html}" /></td>
            </tr>
            <tr>
                <td>{$LANG.COMMENT_FOR_POLL}: </td>
                <td><input name="poll[desc]" type="text" size="30" value="{$thread_poll.description|escape:html}" /></td>
            </tr>
            <tr>
                <td>{$LANG.LENGTH_POLL}: </td>
                <td><input name="poll[days]" type="text" size="4" value="{$thread_poll.days_left}" /> {$LANG.DAYS}</td>
            </tr>
            <tr>
            ...
            <div id="pa_entry{$smarty.section.foo.index}" style="{$style}">
                <table cellspacing="0" class="forum_fa_entry" cellpadding="5">
                    <tr>
                        ...
                        <td><input name="poll[answers][]" type="text" size="30" id="pa_entry_input{$smarty.section.foo.index}" value="{$thread_poll.answers_key[$smarty.section.foo.index]|escape:html}" /></td>
...
<div style="margin-top:6px;">
    <input type="button" class="btn" value="{$LANG.SEND}" onclick="$(this).attr('disabled', 'disabled');$('#msgform').submit();" style="font-size:16px"/>
    <input type="button" class="btn" value="{$LANG.CANCEL}" style="font-size:16px" onclick="window.history.go(-1)"/>
