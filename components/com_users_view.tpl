...
{* ================================================================================ *}
{if $cfg.sw_search}
<div id="users_search_link" class="float_bar"><i class="icon-search"></i> <a href="javascript:void(0)" onclick="{literal}$('#users_sbar').slideToggle('fast');{/literal}"> <span>{$LANG.USERS_SEARCH}</span> </a> </div>
{/if}
...
<div id="users_sbar" {if !$stext}style="display:none;"{/if}>
  <form name="usr_search_form" method="post" action="/users">
    ...
    <p>
      <input name="gosearch" type="submit" class="btn" id="gosearch" value="{if $stext}{$LANG.SEARCH_IN_RESULTS}{else}{$LANG.SEARCH}{/if}" />
      {if $stext}
      	<input type="button" class="btn" value="{$LANG.CANCEL_SEARCH_SHOWALL}" onclick="centerLink('/users/all.html')" />
      {/if}
      <input name="hide" type="button" class="btn" id="hide" value="{$LANG.HIDE}" onclick="{literal}$('#users_sbar').slideToggle();{/literal}"/>
    </p>
  </form>
</div>
{/if}
...
{* ========================= СПИСОК ПОЛЬЗОВАТЕЛЕЙ ============================*}
  <div class="users_list_buttons">
    <div class="btn {if $link.selected=='latest'}active{/if}"><a rel=”nofollow” href="{$link.latest}">{$LANG.LATEST}</a></div>
    <div class="btn {if $link.selected=='positive'}active{/if}"><a rel=”nofollow” href="{$link.positive}">{$LANG.POSITIVE}</a></div>
    <div class="btn {if $link.selected=='rating'}active{/if}"><a rel=”nofollow” href="{$link.rating}">{$LANG.RATING}</a></div>
  </div>
  <div class="users_list">
    <table width="100%" cellspacing="0" cellpadding="0" class="users_list">
      ...
      <tr>
        <td width="80" valign="top"><div class="avatar"><a href="{profile_url login=$usr.login}"><img class="usr_img_small" src="{$usr.avatar}" /></a></div></td>
        <td valign="top">
          ...
          {if $usr.microstatus}
          <div class="microstatus"><i class="icon-comment"></i> {$usr.microstatus}</div>
          {/if} </td>
      </tr>
      {/foreach}
      {else}
