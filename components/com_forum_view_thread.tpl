...
<table class="posts_table" width="100%" cellspacing="2" cellpadding="5" border="0" bordercolor="#999999">
    ...
    <tr>
        <td colspan="2" class="darkBlue-LightBlue">
            <div class="post_date">{if $post.pinned && $num > 1}<i class="icon-bookmark"></i>  {/if}<strong><a name="{$post.id}" href="/forum/thread{$thread.id}-{$page}.html#{$post.id}">#{$num}</a></strong> - {$post.fpubdate}, {$post.wday}</div>
    ...
    <tr class="posts_table_tr">
                ...
                {if $post.user_awards}
                    <div class="post_userawards">
                        {foreach key=aid item=award from=$post.user_awards}
                            <img src="/images/icons/award.gif" alt="{$award.title|escape:html}" title="{$award.title|escape:html}"/>
                        {/foreach}
                    </div>
                {/if}
            </div>
            <div class="hint  hint--top" data-hint="{$LANG.MESSAGES}: {$post.post_count}" class="post_usermsgcnt"><i class="bac-message"></i> {$post.post_count}</div>
            {if $post.city}
        ...
        {else}
            <div class="votes_links" id="votes{$post.id}">
                ...
                <td><a href="javascript:void(0);" onclick="forum.votePost({$post.id}, -1);"><i class="icon-thumbs-down"></i></a></td>
                <td><a href="javascript:void(0);" onclick="forum.votePost({$post.id}, 1);"><i class="icon-thumbs-up"></i></a></td>
                </tr></table>
            </div>
        {/if}
...
<div class="forum_fast">
    <div class="forum_fast_header"><i class="icon-pencil"></i> {$LANG.FAST_ANSWER}</div>
    {if $user_id}
        {if $cfg.fast_bb}
        ...
        <div class="forum_fast_form">
            <form action="/forum/reply{$thread.id}.html" method="post" id="msgform">
                ...
                <div class="forum_fast_submit" style="float:right;padding:5px;"><input type="button" class="btn" value="{$LANG.SEND}" onclick="$(this).attr('disabled', 'disabled');$('#msgform').submit();" /></div>
