...
<div id="shop_toollink_div">
	<a id="shop_searchlink" href="/catalog/{$cat.id}/search.html"><i class="bac-search"></i> {$LANG.SEARCH_BY_CAT}</a>
	{if $cat.view_type=='shop'} {$shopcartlink}	{/if}
    {if $is_can_add}
    <a id="shop_addlink" href="/catalog/{$cat.id}/add.html"><i class="bac-pluse"></i> {$LANG.ADD_ITEM}</a>
    {/if}
</div>
						...
						<td class="uc_list_itemdesc" align="left" valign="top">
                            {if $item.can_edit}
                                <div class="uc_item_edit">
                                    <a href="/catalog/{$cat.id}/edit{$item.id}.html" class="uc_item_edit_link"><i class="icon-pencil"></i> {$LANG.EDIT}</a>
                                </div>
                            {/if}
							<div>
