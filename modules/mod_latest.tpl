	...
	<div class="mod_latest_entry">
		...
		{if $cfg.showdate}
            <div class="mod_latest_date">
                {$article.fpubdate} - <a href="{profile_url login=$article.user_login}">{$article.author}</a>{if $cfg.showcom} - <a href="{$article.url}" title="{$article.comments|spellcount:$LANG.COMMENT1:$LANG.COMMENT2:$LANG.COMMENT10}"><i class="icon-comment"></i> {$article.comments}</a>{/if} - <span><i class="icon-eye-open"></i> {$article.hits}</span>
            </div>
        {/if}
        {if $cfg.showdesc}
...
{if $cfg.showrss}
	<div class="mod_latest_rss">
		<a href="/rss/content/{if $cfg.cat_id}{$cfg.cat_id}{else}all{/if}/feed.rss"><i class="bac-share"> </i> {$LANG.LATEST_RSS}</a>
	</div>
{/if}
