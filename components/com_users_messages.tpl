{if $friends}
    <div class="float_bar">
        <a href="javascript:void(0)" class="new_link" onclick="users.sendMess(0, 0, this);return false;" title="{$LANG.NEW_MESS}:"><span class="ajaxlink"><i class="icon-asterisk"></i> {$LANG.WRITE}</span></a>
    </div>
{/if}
...
<div class="usr_msgmenu_tabs">
    {if $opt == 'in'}
        <span class="usr_msgmenu_active in_span"><i class="icon-envelope"></i> {$page_title} {if $new_messages.messages}({$new_messages.messages}){/if}</span>
    ...
    {elseif $opt == 'out'}
        <a class="usr_msgmenu_link in_link" href="/users/{$id}/messages.html"><i class="icon-envelope"></i> {$LANG.INBOX} {if $new_messages.messages}({$new_messages.messages}){/if}</a>
    ...
    <div class="usr_msg_entry" id="usr_msg_entry_id_{$record.id}">
        ...
        <tr>
            ...
            {if $record.is_new}
                {if $opt=='in' || $opt == 'notices'}
                    <td class="usr_msg_title" width="90" align="right"><span class="msg_new"><i class="icon-asterisk"></i> {$LANG.NEW}!</span></td>
                {else}
                    <td class="usr_msg_title" width="90" align="right"><a class="msg_delete" href="javascript:void(0)" onclick="users.deleteMessage('{$record.id}')"><span class="ajaxlink"><i class="icon-minus-sign"></i> {$LANG.CANCEL_MESS}</span></a></td>
                {/if}
            {else}
            ...
            {if $opt == 'in' || (in_array($opt, array('out','history','notices')) && !$record.is_new)}
                <td class="usr_msg_title" width="70" align="right"><a class="msg_delete" href="javascript:void(0)" onclick="users.deleteMessage('{$record.id}')"><span class="ajaxlink"><i class="icon-minus-sign"></i> {$LANG.DELETE}</span></a></td>
            {/if}
        </tr>
        </table>
        <table cellspacing="4">
        <tr>
            <td width="70" height="70" valign="middle" align="center" style="border:solid 1px #C3D6DF; padding: 4px">
                ...
                {else}
                    <img class="usr_img_small" src="{$record.user_img}" />
                {/if}
