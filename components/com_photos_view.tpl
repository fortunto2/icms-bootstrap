...
{elseif $album.id != $root_album_id && $album.orderform}
    ...
    <div class="float_bar">
        <form action="" method="POST" style="float: left">
            {$LANG.SORTING_PHOTOS}: 
            <select name="orderby" id="orderby" class="span2">
            ...
            <select name="orderto" id="orderto" class="span2">
            ...
            </select><input type="submit" class="btn sort" value=">>" />
        </form>
        {if $can_add_photo}
...
{if $album.description}
	<p class="usr_photos_notice"><i class="icon-info-sign"></i> {$album.description|nl2br}</p>
{/if}
{if $subcats}
    ...
    {foreach key=tid item=photo from=$photos}
        ...
        <td align="center" valign="middle" width="{math equation="100/x" x=$album.maxcols}%">
            <div class="{$album.cssprefix}photo_thumb" align="center">
                ...
                <a href="/photos/photo{$photo.id}.html" title="{$photo.title|escape:'html'}">{$photo.title|truncate:25}</a>
                {if $album.showdate}
