{if $myprofile}
<div class="float_bar">
    <a href="/users/addfile.html" class="add_file_link"><i class="icon-upload"></i> {$LANG.UPLOAD_FILE_IN_ARCHIVE}</a>
</div>
{/if}
      ...
      {if $total_files > 1}
        <td align="right">
            <form name="orderform" method="post" action="" style="margin:0px">
                <input type="button" class="btn" class="usr_files_orderbtn" onclick="orderPage('pubdate')" name="order_date" value="{$LANG.ORDER_BY_DATE}" {if $orderby=='pubdate'} disabled {/if} />
                <input type="button" class="btn" class="usr_files_orderbtn" onclick="orderPage('filename')" name="order_title" value="{$LANG.ORDER_BY_NAME}" {if $orderby=='filename'} disabled {/if} />
                <input type="button" class="btn" class="usr_files_orderbtn" onclick="orderPage('filesize')" name="order_size" value="{$LANG.ORDER_BY_SIZE}" {if $orderby=='filesize'} disabled {/if} />
                <input type="button" class="btn" class="usr_files_orderbtn" onclick="orderPage('hits')" name="order_hits" value="{$LANG.ORDER_BY_DOWNLOAD}" {if $orderby=='hits'} disabled {/if} />
...
<form name="listform" id="listform" action="" method="post">
  ...
  {if $myprofile || $is_admin}
    <div style="margin-top:6px; float:right;">
      <input type="button" class="btn" class="usr_files_orderbtn" name="delete_btn" id="delete_btn" onclick="delFiles('{$LANG.YOU_REALLY_DEL_FILES}?')" value="{$LANG.DELETE}"/>
      <input type="button" class="btn" class="usr_files_orderbtn" name="hide_btn" id="delete_btn" onclick="pubFiles(0)" value="{$LANG.HIDE}"/>
      <input type="button" class="btn" class="usr_files_orderbtn" name="show_btn" id="delete_btn" onclick="pubFiles(1)" value="{$LANG.SHOW}"/>
    </div>
  {/if}
  {$pagination}
</form>
