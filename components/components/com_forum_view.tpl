...
{if $threads}
    ...
    {foreach key=id item=thread from=$threads}
        ...
        <tr>
            {if $thread.pinned}
                <td width="30" class="{$class}" align="center" valign="middle"><img alt="{$LANG.ATTACHED_THREAD}" src="/templates/{template}/images/icons/forum/pinned.png" title="{$LANG.ATTACHED_THREAD}" /></td>
            {else}
                {if $thread.closed}
                    <td width="30" class="{$class}" align="center" valign="middle"><img alt="{$LANG.THREAD_CLOSE}" src="/templates/{template}/images/icons/forum/closed.png" title="{$LANG.THREAD_CLOSE}" /></td>
                {else}
                    {if $thread.is_new}
                        <td width="30" class="{$class}" align="center" valign="middle"><i title="{$LANG.HAVE_NEW_MESS}" class="icon-asterisk"></i></td>
                    {else}
                        <td width="30" class="{$class}" align="center" valign="middle"><img alt="{$LANG.NOT_NEW_MESS}" src="/templates/{template}/images/icons/forum/old.png"  title="{$LANG.NOT_NEW_MESS}" /></td>
                    {/if}
                {/if}
            {/if}
            ...
            <td width="200" style="font-size:12px" class="{$class}">
                {if $thread.last_msg_array}
                    <a href="/forum/thread{$thread.last_msg_array.thread_id}-{$thread.last_msg_array.lastpage}.html#{$thread.last_msg_array.id}"><i class="icon-arrow-right"></i></a>
            ...
            <form action="" method="post">
                ...
                <tbody>
                    <tr valign="bottom">
                      ...
                      <td>
                        <div></div>
                        <input type="submit" class="btn" value="{$LANG.SHOW_THREADS}">
                      </td>
                    </tr>
                </tbody>
            </table>
