{if $is_uc}
    ...
    {if $cfg.showtype == 'list'}
		<table width="100%" class="table table-bordered table-hover">
		    {foreach key=tid item=item from=$items}
				<tr>
					<td><a class="uc_latest_link" href="/catalog/item{$item.id}.html">{$item.title}</a></td>	
	...
	{/if}
	{if $cfg.fulllink}
	    <div style="text-align:right; clear:both"><a style="text-decoration:underline" href="/catalog">{$LANG.UC_POPULAR_CATALOG}</a> {$LANG.UC_POPULAR_ARR}</div>
	{/if}
{else}
