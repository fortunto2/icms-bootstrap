...
<div class="large-photo">
	...
	<img src="/images/photos/medium/{$photo.file}" alt="{$photo.title|escape:'html'}" class="span12 usr_img" id="profile-avatar">
	</div>
...
<span class="span2" id="karmapoints"><sdivong>{$LANG.RATING}: </sdivong>{$photo.rating|rating}</span> {if $photo.karma_buttons}<span>{$photo.karma_buttons}</span>{/if} <span class="span3"><sdivong>{$LANG.HITS}: </sdivong> {$photo.hits}</span>
                ...
                <div class="span6">
                    <span>{if !$photo.published}<span id="pub_photo_wait" style="color:#F00;">{$LANG.WAIT_MODERING}</span><span id="pub_photo_date" style="display:none;">{$photo.pubdate_dop}</span>{else}{$photo.pubdate_dop}{/if} </span>
