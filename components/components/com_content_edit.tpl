...
<form id="addform" name="addform" method="post" action="" enctype="multipart/form-data">
			...
			<td>
				<table width="700" border="0" cellspacing="5" class="proptable">
					...
					<tr>
						...
						<td valign="top">
							<input name="tags" type="text" value="{$mod.tags|escape:'html'}" />
	...
	<p style="margin-top:15px">
		...
		<input name="savebtn" type="button" class="btn" onclick="submitArticle()" id="add_mod" {if $do=='addarticle'} value="{$LANG.ADD_ARTICLE}" {else} value="{$LANG.SAVE_CHANGES}" {/if} />
		<input name="back" type="button" class="btn" id="back" value="{$LANG.CANCEL}" onclick="window.history.back();"/>
		{if $do=='editarticle'}
