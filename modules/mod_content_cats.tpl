...
<ul class="mod_cat_list">
    ...
    {foreach key=tid item=cat from=$subcats_list}
        ...
        {/if}
        {if $cat.NSLevel <= 1}
            <i class="icon-folder-open"></i> <a class="folder" href="{$cat.url}">{if $cat.seolink == $current_seolink} <strong>{$cat.title} ({$cat.content_count})</strong>{else}{$cat.title} ({$cat.content_count}){/if}</a>
        {else}
